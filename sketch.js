...
class Obstacle {
  constructor(type, xPos) {
    ...
    if (type === 'low') {
      ...
      this.width = 150; //platformlengte aanpassen
      this.height = 15;
...
class DinoGame {
 ...
 reset() {
  ...
  spawnObstacles() {
    ...
    if (rand < 0.4) {  // 40% kans: laag obstakel
      this.obstacles.push(new Obstacle('low', MARGIN + COLS * CELL_SIZE));
    } else if (rand < 0.7) {  // 30% kans: hoog obstakel
      this.obstacles.push(new Obstacle('high', MARGIN + COLS * CELL_SIZE));
    } else {    // 30% kans: platform (alle 3 samen 100%)
      let platform = new Obstacle('platform', MARGIN + COLS * CELL_SIZE);
      this.obstacles.push(platform); // Platform met obstakel erachter
      let followUp = new Obstacle(random() < 0.5 ? 'low' : 'high', MARGIN + COLS * CELL_SIZE + 250); //obstakel dichterbij platform 100, verderweg 150 >
  ...
  update(gameY) {
    ...
    this.spawnTimer++; //60, 100 minderen = vaker obstakels
